...
/* Located by scanning the service rather than hardcoded like the reports
 * above: which index it lands on depends on which optional reports are
 * compiled in, so arithmetic would break silently as those options change. */
static const struct bt_gatt_attr *gamepad_attr;

static const struct bt_gatt_attr *find_gamepad_attr(void) {
    if (gamepad_attr != NULL) {
        return gamepad_attr;
    }

    for (uint16_t i = 0; i < hog_svc.attr_count; i++) {
        if (hog_svc.attrs[i].read == read_hids_gamepad_input_report) {
            gamepad_attr = &hog_svc.attrs[i];
            return gamepad_attr;
        }
    }

    LOG_ERR("Could not locate the gamepad report attribute");
    return NULL;
}
...
void send_gamepad_report_callback(struct k_work *work) {
    ...
    while (k_msgq_get(&zmk_hog_gamepad_msgq, &report, K_NO_WAIT) == 0) {
        const struct bt_gatt_attr *attr = find_gamepad_attr();
        if (attr == NULL) {
            return;
        }
