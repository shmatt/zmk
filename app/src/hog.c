...
/* Found by scanning the service rather than hardcoded like the reports above:
 * the index depends on which optional reports are compiled in, so an
 * arithmetic guess would break silently as those options change. */
