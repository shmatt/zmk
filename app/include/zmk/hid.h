...
#error "A proper consumer HID report usage range must be selected"
    ...
    HID_INPUT(ZMK_HID_MAIN_VAL_DATA | ZMK_HID_MAIN_VAL_VAR | ZMK_HID_MAIN_VAL_ABS),
    /* A hat switch, because a D-pad reported as four buttons does not
     * produce D-pad events on hosts -- they derive those only from a hat.
     * Four bits carry the eight directions, with the null state marking
     * neutral, then four bits of padding to finish the byte. */
    HID_USAGE_PAGE(HID_USAGE_GEN_DESKTOP),
...
struct zmk_hid_gamepad_report_body {
     ...
     * descriptor's null state) meaning centred. Appended last so the fields
...
/* Directions as booleans; opposing presses cancel, which is what a hat can
